...
namespace JoeAbp.Tests
{
    public abstract class JoeAbpTestBase : AbpIntegratedTestBase<JoeAbpTestModule>
    {
        ...
        protected void UsingDbContext(Action<JoeAbpDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<JoeAbpDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<JoeAbpDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<JoeAbpDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<JoeAbpDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<JoeAbpDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<JoeAbpDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<JoeAbpDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<JoeAbpDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<JoeAbpDbContext, Task<T>> func)
        {
            T result;
